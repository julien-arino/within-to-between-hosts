#!/usr/bin/env Rscript
# ============================================================
# File: plot-Figure-C2a-boundIFN-vs-viral-load.R
# Description:
#   Plot max(F_B) against max(V) for the cohort
# ============================================================

project_dir <- here::here()
source(file.path(project_dir, "CODE", "functions-and-definitions.R"))

start_time <- start_time_and_hello("plot-Figure-C2a-boundIFN-vs-viral-load.R")
...
# 5. Save to PDF & PNG
out_pdf <- file.path(figs_dir, "Figure-C2a-boundIFN-vs-viral-load.pdf")
out_png <- file.path(figs_dir, "Figure-C2a-boundIFN-vs-viral-load.png")

ggsave(filename = out_pdf, plot = p_FB, width = 6, height = 5, units = "in", dpi = 300)
ggsave(filename = out_png, plot = p_FB, width = 6, height = 5, units = "in", dpi = 300)

cat("Plots saved to:\n  -", out_pdf, "\n  -", out_png, "\n")
print_end_time(start_time, "plot-Figure-C2a-boundIFN-vs-viral-load.R")
