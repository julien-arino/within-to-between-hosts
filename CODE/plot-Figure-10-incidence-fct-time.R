...
start_time <- start_time_and_hello("plot-Figure-10-incidence-fct-time.R")
...
combined_df <- dist_df %>%
  select(time, beta_mean, beta_q90 = beta_Q90, gamma_P, mu_P) %>%
  filter(time <= 50) %>%
  mutate(
...
out_pdf <- file.path(figs_dir, "Figure-10-incidence-fct-time.pdf")
out_png <- file.path(figs_dir, "Figure-10-incidence-fct-time.png")
...
cat("Between-host simulation plot saved:\n -", out_pdf, "\n -", out_png, "\n")

print_end_time(start_time, "plot-Figure-10-incidence-fct-time.R")
