...
r_script_path = joinpath(SCRIPT_DIR, "create-sample-for-sensitivity-withV0.R")
...
# Note: all output filenames have been modified to include `_withV0` so they don't overwrite standard datasets.
## Save the results as a JLS file
if SAVE_JLS
    println("Saving results as JLS")
    save_path = joinpath(OUTPUT, @sprintf("sensitivity_P%07d_DT%s_%s_withV0.jls", N, date_time_start, type_output))
...
if SAVE_RDS
    ...
    save_path_rds = joinpath(OUTPUT, @sprintf("sensitivity_P%07d_DT%s_%s_withV0.Rds", N, date_time_start, type_output))
...
if SAVE_QS
    ...
    save_path_qs = joinpath(OUTPUT, @sprintf("sensitivity_P%07d_DT%s_%s_withV0.qs", N, date_time_start, type_output))
    @rput SAVE          # Send the Julia object to R
...
if SAVE_CSV
    ...
    save_path_csv = joinpath(OUTPUT, @sprintf("sensitivity_P%07d_DT%s_%s_withV0.csv", N, date_time_start, type_output))
