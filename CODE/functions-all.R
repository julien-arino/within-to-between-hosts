...
##
# The right hand side of the within host ODE. In this version, some parameters
...
set_parameters <- function() {
  ...
  # | c_star     | Initial CD8+ T cells |  1.104E-4 |  $10^9$cells/ml |
  params <- c(
    lambda_S = 0.74,
    S_max = 0.16,
    d_I = 0.1,
...
generate_params_patients <- function(params, n = 1000) {
  ...
  colnames(OUT) <- names_params[setdiff(
    1:length(names_params),
    idx_stddev
  )]
...
equilibrium_F <- function(params) {
  psi_F_prod <- params[["psi_F_prod"]]
  ...
  FB <- (psi_F_prod - k_lin_f * FU) / k_int_f

  return(list(FU = FU, FB = FB))
}

# Compute theta^0 in R0 formula
theta_zero <- function(FB, params) {
  epsilon_FI <- params[["epsilon_FI"]]
  return(epsilon_FI / (epsilon_FI + FB))
}

# Compute R0
reproduction_number <- function(params) {
  eq_F <- equilibrium_F(params)
