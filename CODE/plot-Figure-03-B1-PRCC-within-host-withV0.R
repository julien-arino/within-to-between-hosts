# ============================================================
# File : plot-Figure-03-PRCC-within-host-withV0.R
# category-based plots for PRCC values including V0
# ============================================================
...
# Load data - Specifically target the withV0 datasets!
prcc_data <- qs_read(file.path(OUTPUT, "PRCC_results_withV0.qs"))
PRCC_vals <- prcc_data$PRCC_vals
PRCC_times <- prcc_data$PRCC_times
PRCC_global <- read.csv(file.path(OUTPUT, "PRCC_global_summary_withV0.csv"))

# ------------------------------------------------------------
# 8. Plot configuration
# ------------------------------------------------------------
# Added V0 to cat_viral!
...
print(P_max)
ggsave(
    filename = file.path(FIGS, "Figure-B1a-PRCC-values-withV0.png"),
    plot = P_max, width = 25, height = 15, units = "cm", dpi = 300
)
ggsave(
    filename = file.path(FIGS, "Figure-B1a-PRCC-values-withV0.pdf"),
...
print(P_tmax)
ggsave(
    filename = file.path(FIGS, "Figure-B1b-PRCC-values-times-withV0.png"),
    plot = P_tmax, width = 25, height = 15, units = "cm", dpi = 300
)
ggsave(
    filename = file.path(FIGS, "Figure-B1b-PRCC-values-times-withV0.pdf"),
...
cat("\nGlobal PRCC plot saved:", file.path(FIGS, "Figure-03-PRCC-global-withV0.pdf"), "\n")
