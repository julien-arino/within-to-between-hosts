#!/usr/bin/env Rscript
# ============================================================
# File: plot-Figure-C6b-tau-max-viral-load-vs-V0.R
# Description:
#   Plot time to maximum viral load per individual against initial viral load (V0), colored by status
# ============================================================

project_dir <- here::here()
source(file.path(project_dir, "CODE", "functions-and-definitions.R"))

start_time <- start_time_and_hello("plot-Figure-C6b-tau-max-viral-load-vs-V0.R")
...
# ------------------------------------------------------------
viral_tau_df <- cohort_df %>%
  select(ID, status, tau_max_V) %>%
  filter(tau_max_V < 100)

v0_df <- params_df %>%
  select(ID, V0)

plot_df <- inner_join(viral_tau_df, v0_df, by = "ID")
...
# 4. Create the plot
# ------------------------------------------------------------
p_scatter <- ggplot(plot_df, aes(x = V0, y = tau_max_V, color = status)) +
  geom_point_rast(alpha = 0.3, size = 1) +
  scale_color_manual(values = status_colors) +
  labs(
    x = expression("log"[10]*"(Initial viral load (V"[0]*") [copies/ml])"),
    y = expression("Time to maximum viral load ("*tau[max[V]]*") [days]"),
    color = "Status"
  ) +
...
# ------------------------------------------------------------
out_pdf <- file.path(figs_dir, "Figure-C6b-tau-max-viral-load-vs-V0.pdf")
out_png <- file.path(figs_dir, "Figure-C6b-tau-max-viral-load-vs-V0.png")
...
print_end_time(start_time, "plot-Figure-C6b-tau-max-viral-load-vs-V0.R")
